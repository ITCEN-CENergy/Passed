...
ALTER TABLE roadmaps DROP COLUMN job_posting_id;
ALTER TABLE roadmaps DROP COLUMN report_id;
ALTER TABLE roadmaps DROP COLUMN last_replanned_at;
ALTER TABLE roadmaps ADD COLUMN failure_reason TEXT;
...
CREATE INDEX idx_roadmaps_user_created_id
    ON roadmaps(user_id, created_at DESC, id DESC);

CREATE TABLE roadmap_job_postings (
...
CREATE INDEX idx_roadmap_job_postings_job_posting
...
ALTER TABLE roadmap_skills DROP COLUMN skill_id;
ALTER TABLE roadmap_skills DROP COLUMN importance;
ALTER TABLE roadmap_skills DROP COLUMN current_level;
ALTER TABLE roadmap_skills DROP COLUMN target_level;
ALTER TABLE roadmap_skills ADD COLUMN standard_competency_id BIGINT;
ALTER TABLE roadmap_skills ADD COLUMN standard_competency_name VARCHAR(200);
ALTER TABLE roadmap_skills ADD COLUMN category VARCHAR(50);
ALTER TABLE roadmap_skills ADD COLUMN current_level INT;
ALTER TABLE roadmap_skills ADD COLUMN target_level INT;
ALTER TABLE roadmap_skills ADD COLUMN requirement_type VARCHAR(50);
ALTER TABLE roadmap_skills ADD COLUMN gap_level INT;
ALTER TABLE roadmap_skills ADD COLUMN frequency INT;
ALTER TABLE roadmap_skills ADD COLUMN priority_score NUMERIC(10, 4);
...
CREATE INDEX idx_roadmap_skills_competency ON roadmap_skills(standard_competency_id);

CREATE TABLE roadmap_skill_sources (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE INDEX idx_roadmap_skill_sources_job_posting ON roadmap_skill_sources(job_posting_id);
...
ALTER TABLE milestones DROP COLUMN skill_id;
ALTER TABLE milestones ADD COLUMN standard_competency_id BIGINT;
ALTER TABLE milestones ADD COLUMN learning_objective TEXT;
ALTER TABLE milestones ADD COLUMN start_level INT;
ALTER TABLE milestones ADD COLUMN target_level INT;
ALTER TABLE milestones ADD COLUMN milestone_type VARCHAR(50);
...
CREATE INDEX idx_milestones_user_competency ON milestones(user_id, standard_competency_id);
...
ALTER TABLE roadmap_milestones ADD COLUMN reuse_type VARCHAR(50) NOT NULL DEFAULT 'NEW';
ALTER TABLE roadmap_milestones ADD COLUMN reuse_reason TEXT;
ALTER TABLE roadmap_milestones ADD COLUMN is_required BOOLEAN NOT NULL DEFAULT TRUE;
