...
const JobPostingBasket = () => {
  ...
  return <>
    ...
    {open && <section className={styles.panel} aria-label="학습 로드맵 공고함">
      ...
      <div className={styles.listHeader}><strong>선택한 공고 {items.length}개</strong><button type="button" disabled={!items.length} onClick={clearItems}>전체 비우기</button></div>
      <div className={styles.list}>
        {items.length ? items.map(item => <article key={item.jobPostingId}><div><span>{item.companyName}</span><h3>{item.title}</h3></div><button type="button" onClick={() => removeItem(item.jobPostingId)}>삭제</button></article>) : <p className={styles.empty}>선택한 공고가 없어요.<br />로드맵으로 학습할 공고를 담아보세요.</p>}
      </div>
